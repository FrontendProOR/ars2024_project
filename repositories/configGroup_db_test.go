...
func TestConfigGroupDBRepository_Add(t *testing.T) {
...
func TestConfigGroupDBRepository_Delete(t *testing.T) {
	// Create a new database instance
	db, err := data.NewDatabase()
	assert.NoError(t, err)
	...
	// Retrieve the config group from the repository
	_, err = repo.Get(configGroup.Name, configGroup.Version)
	assert.Error(t, err)
}

func TestConfigGroupDBRepository_Get(t *testing.T) {
	// Create a new database instance
	db, err := data.NewDatabase()
	assert.NoError(t, err)
